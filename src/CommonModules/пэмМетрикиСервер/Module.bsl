...
Процедура СформироватьМетрикиРегламентнымЗаданием(Метрика = Неопределено) Экспорт
	...
	Если КодыСостоянияHTTPКлиентСерверПовтИсп.ЭтоОшибкаКлиента(Результат.КодСостояния)
		ИЛИ КодыСостоянияHTTPКлиентСерверПовтИсп.ЭтоОшибкаСервера(Результат.КодСостояния) Тогда
...
Процедура СформироватьМетрикуФоновымЗаданием(ИмяМетрики, ТипМетрики, Алгоритм, ИдентификаторХранилища, Метрика) Экспорт
	...
	ПараметрыАлгоритма = Справочники.пэмМетрики.ПараметрыАлгоритмаМетрики(Метрика);
	
	сткВозврат = СформироватьМетрику(Алгоритм, ПараметрыАлгоритма);
	...
	ПоместитьВоВременноеХранилище(сткВозврат, ИдентификаторХранилища);
	ЗаписатьИнформациюОРасчетеМетрикиФоновымЗаданием(Метрика, НачалоРасчета, ДатаНачалаРасчета);
	
КонецПроцедуры

Функция СформироватьМетрику(Алгоритм, Параметры) Экспорт
...
// TODO спорная реализация работы с регламентными заданиями
Процедура ОбновитьРегламентноеЗадание(ПараметрыРегламентногоЗадания) Экспорт

	УстановитьПривилегированныйРежим(Истина);
	
	Если УдалятьРегламентноеЗадание(ПараметрыРегламентногоЗадания) Тогда
...
Функция УдалятьРегламентноеЗадание(Знач Параметры)
	
	Возврат НЕ Параметры.Предопределенное
			И ЗначениеЗаполнено(Параметры.УникальныйИдентификатор) 
			И НЕ Параметры.Использование;
		
КонецФункции

Функция ФоновоеЗаданиеАктивно(Знач Идентификатор)
...
Функция СформироватьМетрикиСинхронно(Выборка, МетодПолученияМетрики)
	...
	Пока Выборка.Следующий() Цикл
		...
		ПараметрыАлгоритма = Справочники.пэмМетрики.ПараметрыАлгоритмаМетрики(Выборка.Метрика);
		
		сткВозврат = СформироватьМетрику(Выборка.Алгоритм, ПараметрыАлгоритма);
		Если сткВозврат.Ошибка Тогда
			Продолжить;
		КонецЕсли;
		...
		ЗаписатьИнформациюОРасчетеМетрикиФоновымЗаданием(Выборка.Метрика, НачалоРасчета, ДатаНачалаРасчета);
...
Функция СформироватьМетрикиАсинхронно(Выборка, МетодПолученияМетрики)
	...
	Пока Выборка.Следующий() Цикл
		...
		Параметры.Очистить();
		Параметры.Добавить(Выборка.ИмяМетрики);
		Параметры.Добавить(Выборка.ТипМетрики);
		Параметры.Добавить(Выборка.Алгоритм);
		Параметры.Добавить(АдресВХранилище);
		Параметры.Добавить(Выборка.Метрика);
		
		ФоновоеЗадание = ФоновыеЗадания.Выполнить(
			"пэмМетрикиСервер.СформироватьМетрикуФоновымЗаданием", Параметры, АдресВХранилище);
		МассивФоновыхЗаданий.Добавить(ФоновоеЗадание);
		
	КонецЦикла;
	
	Если НЕ МассивФоновыхЗаданий.Количество() Тогда // TODO какокод, разобраться что хотел автор и заменить
...
Процедура ЗаписатьИнформациюОРасчетеМетрикиФоновымЗаданием(Метрика, НачалоРасчета, ДатаНачалаРасчета)
	
		Параметры = Новый Массив;
		Параметры.Добавить(Метрика);
		Параметры.Добавить(НачалоРасчета);
		Параметры.Добавить(ТекущаяУниверсальнаяДатаВМиллисекундах());
		Параметры.Добавить(ДатаНачалаРасчета);
		
		ФоновыеЗадания.Выполнить("пэмМетрикиСервер.ЗаписатьИнформациюОРасчетеМетрики", Параметры);	
...
Процедура УдалитьРегламентноеЗаданиеПоИдентификатору(Знач Идентификатор)
